    ...
    foreach ($_POST['checkBoxArray'] as $postValueId) {
        ...
        switch ($bulk_options) {
            ...
            case 'draft':
                ...
                break;
            case 'delete':
                $query = "DELETE FROM posts where posts.post_id = '{$postValueId}' ";
                $update_to_published_status = mysqli_query($connection, $query);
                comfirm($update_to_published_status);
                break;
            case 'clone':
                $query = "SELECT * FROM posts where posts.post_id = '{$postValueId}' ";
...
if (isset($_GET['reset'])) {
    ...
    $query = "UPDATE `posts` SET `post_views_count` = 0 WHERE `posts`.`post_id` = ".mysqli_real_escape_string($connection,$_GET['reset']);
